...
function generateInvoicePdf(order: OrderResponse, callBack: (filePath: string, fileName: string) => void) {
  ...
  order.productsOrdered.products.forEach((product) => {
    pdfDoc
      .fontSize(16)
      .text(
        alignText(product.name, col1Width) +
...
const alignText = (text: string | number, width: number, alignment?: string) => {
